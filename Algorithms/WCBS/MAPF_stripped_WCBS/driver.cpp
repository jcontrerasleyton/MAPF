...
//  }
  ...
  string file = "../Results/wcbs_Input_"+std::to_string(al.num_of_agents)+"-w_"+std::to_string(w_window_limit)+".csv";
  ...
  done = cbs.checkGoal(); // Validate if every agent reach its goal
  ...
  //cbs.solution_cost = cbs.solution_cost - (iteration*cbs.num_of_agents);

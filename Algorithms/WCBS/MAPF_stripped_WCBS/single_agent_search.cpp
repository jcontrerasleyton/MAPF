...
bool SingleAgentSearch::findPath(const vector < list< pair<int, int> > >* constraints, int w_window_limit) {
  ...
  while ( !open_list.empty() ) {
    ...
    //if ((curr->id == goal_location && curr->timestep > lastGoalConsTime) || curr->timestep == w_window_limit) {
    if (w_window_limit == -1){

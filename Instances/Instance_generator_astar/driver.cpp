...
int main(int argc, char** argv) {
  ...
  for(int i = first; i < total_agents; i++){
    ...
    while(total < instances){
      ...
      string path = "../../Instances/Input/"+name+"/Input_"+std::to_string(first);
      ...
      while(subtotal < i){
        ...
        if(init_loc != goal_loc && aux_ini[init_loc] == false && aux_goal[goal_loc] == false){
          ...
          if(h_values[init_loc] == DBL_MAX) continue; //If the start node cant reach goal, the program omits this start-goal pair.
  ...
  cout << endl << "All instances (" << first << " agents) generated" << endl;
  cout << endl << "--------------------------------------------" << endl;

...
int main(int argc, char** argv) {
  ...
  // Reading arguments ----------------------------------------------------------------
  cout << endl;
  string map_fname, agents_fname, name, hwy_fname;
  //, search_method, results_fname;
  double w_hwy;
  // w_focal;
  int ins, agents;
  ...
  while(!done){
    ...
    while(end){ // Search for a random goal until finds a free state
      goal_loc = ml.linearize_coordinate((rand() % fin)+1,(rand() % fin2)+1);
      end = ml.my_map[goal_loc];
    }

    ComputeHeuristic ch(goal_loc, ml.get_map(), ml.rows*ml.cols, ml.actions_offset, w_hwy, &egr);
    double* h_values = ch.getHVals();

    int cont = 0;

    aux_copy = new bool[ml.rows*ml.cols];
    aux_map = new int[ml.rows*ml.cols];

    for (int j=0; j < ml.rows*ml.cols; j++){
      if(h_values[j] == DBL_MAX){
        aux_copy[j] = true;
        aux_map[j] = 1;
        cont++;      
      }
    ...
    cout << "Nodes: " << ml.rows*ml.cols << ", Blocked: " << cont0 << " vs " << cont << " ";
    
    double rate = ((double)cont/(double)(ml.rows*ml.cols))*100;
    cout << rate << "%" << endl;

    if(rate < 85 || cont0 == cont) done = true; //Cambio
  }
  
  std::ofstream outfile;
  outfile.open("../AUX/"+name+".map", std::ios_base::trunc);
  outfile << std::to_string(ml.rows)+","+std::to_string(ml.cols)+"\n";
  for (int j=0; j < ml.rows; j++){
    for (int l=0; l < ml.cols; l++){
      outfile << std::to_string(aux_map[ml.linearize_coordinate(j,l)]);
      if (l != ml.cols-1)
        outfile << +",";
    }
    outfile << "\n";
  ...
  for(int i = first; i < total_agents; i++){
    ...
    while(total < instances){
      ...
      for (int j=0; j < ml.rows*ml.cols; j++)
        ...
        aux_ini[j] = aux_copy[j];

      aux_goal = new bool[ml.rows*ml.cols];
      for (int j=0; j < ml.rows*ml.cols; j++)
        //aux_goal[j] = ml.my_map[j];
        aux_goal[j] = aux_copy[j];

      
      string path = "../../Instances/Input_patched/"+name+"/Input_"+std::to_string(first);
